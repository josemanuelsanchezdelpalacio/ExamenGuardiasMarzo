...
fun OpcionesScreen(navController: NavHostController, mvvm: ViewModelOpcionesScreen) {
    Scaffold(
        ...
        opcionesScreenBodyContent(modifier = Modifier.padding(paddingValues), mvvm, navController)
    }
}


@Composable
fun opcionesScreenBodyContent(modifier: Modifier, mvvm: ViewModelOpcionesScreen, navController: NavHostController) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp),
        ...
        Button(onClick = { mvvm.cambiarIconoYNombreDeLaApp() }) {
            Text("Cambiar icono y nombre de la app")
        }
        Button(onClick = { mvvm.establecerContactoDeEmergencia() }) {
            Text("Establecer contacto de emergencia")
        }
        Button(onClick = { mvvm.mostrarVideosExplicativos() }) {
            Text("Videos explicativos")
        }
        Button(onClick = { mvvm.mostrarFormularioParaEmpezar() }) {
            Text("Formulario para empezar")
        }
    }
}